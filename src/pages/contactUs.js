...
const ContactUs = (props = "") => {
  ...
  if (JSON.parse(localStorage.getItem('userEmail'))){ emailW= JSON.parse(localStorage.getItem('userEmail')); }
    ...
    const handleChangeB = (e) => { setPolicy(!e.target.checked); }
    const handleChange = (e) => { setPolicy(!e.target.checked); }
    ...
    const onChange = (e) => { setSelectedFiles(e.target.files) }
    const onClick = () => { setSelectedFiles(null) }
    const onChangeCustomerName = (e) => { setName(e.target.CustomerName); console.log(name) }
    const onChangeEmail = (e) => { setEmail(e.target.email); console.log(email) }
    const onChangePhone = (e) => { setPhone(e.target.phone) }
        ...
        <>
            <Desktop>
                <form id={styles.contactUs} onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <p id={styles.note} >Please leave your details below and we’ll call you back in the next 24 hours</p>

                    <input placeholder="Enter Full Name" id={styles.customerName}
                        className={`${errors.customerName ? styles.errorBorder : styles.Border}`}
                        defaultValue="" type="text" name="customerName" onChange={onChangeCustomerName}
                    ...
                    <input placeholder="Enter phone Number(with country code)" id={styles.phone}
                        className={`${errors.phone ? styles.errorBorder : styles.Border}`}
                        defaultValue="" type="phone" name="phone" onChange={onChangePhone}
                        {...register('phone', {
                            required: "*",
                            pattern: {
                    ...
                    <textarea id={styles.customerMessage} placeholder="Your Message"
                        className={`${errors.customerMessage ? styles.errorBorder : styles.Border}`}
                    ...
                    {errors.phone && <p id={styles.pError} role="alert"  > {errors.phone.message}</p>}

                    {(errors.customerName || errors.customerMail || errors.phone || policy) && <p id={styles.dError} role="alert" > Please fill out all Fields! </p>}
                    <div className={styles.rnc}>
                        <Captcha
                            onChange={onChangeCaptcha}
                        />
                    </div>
                    <button id={styles.submit} className={`${!policy && isSubmitSuccessful ? styles.allow : styles.notallow}`} type="submit"
                        onClick={handleSubmit(onSubmit)} disabled={policy || isSubmitSuccessful}  >{submitStatus}</button>
                </form>
            </Desktop>
        ...
        <Desktop>
            <form id={styles.modalContactUs} onSubmit={handleSubmit(onSubmit)} style={{ backgroundImage: `url(${img})` }} className={ contactFrmShow ? (props.src == 'REQUEST A PROPOSAL' ? styles.fp : (props.src == 'JOIN US' ? styles.fj : styles.modalContactUs)) : styles.inactive} >
                <div className={styles.container}>
                    ...
                    <h1 id={styles.title} ><b> {props.src} </b></h1>
                    ...
                    <input placeholder="Enter phone Number(with country code)" id={styles.phone} I
                        className={`${errors.phone ? styles.errorBorder : styles.Border}`}
                        defaultValue="" type="phone" name="phone" onChange={onChangePhone}
                        {...register('phone', {
                            required: "*",
                    ...
                    <label id={styles.rf} >You Can Upload your Documents</label>
                    <input type="file" placeholder="Your File" id={styles.rfp} name="fileInput"
                        {...register("fileInput")} multiple accept=" .pdf, .jpg, .jpeg, .png, .zip "
                        onChange={onChange} onClick={onClick} />
                    ...
                    <button id={styles.submit} type="submit" className={`${!policy && isSubmitSuccessful ? styles.allow : styles.notallow}`}
                        onClick={handleSubmit(onSubmit)} disabled={policy || isSubmitSuccessful} >{submitStatus}</button>
                </div>
            </form>
            </Desktop>
